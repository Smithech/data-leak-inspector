    ...
    def analyze(self, metadata: FileMetadata) -> ExposureResult:
        ...
        if not permissions:
            return ExposureResult(
                ExposureLevel.PRIVATE, "no explicit permissions found"
            )
        ...
        for p in permissions:
            if p.get("type") == "anyone":
                ...
                return ExposureResult(
                    ExposureLevel.PUBLIC, f"anyone with link ({role})"
                )
        ...
        if len(permissions) > 1:
            ...
            return ExposureResult(
                ExposureLevel.SHARED, f"shared with {user_count} user(s)"
            )

        for p in permissions:
            if p.get("type") in ("domain", "group"):
                return ExposureResult(
                    ExposureLevel.SHARED, f"shared with {p.get('type')}"
                )
